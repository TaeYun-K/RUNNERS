...
export function CommunityPostWritePage() {
  ...
  const { creating, error, create } = useCreateCommunityPost()
  ...
  const canSubmit = useMemo(() => {
    if (creating || uploadingImages) return false
    ...
    return true
  }, [contentLength, creating, images.length, titleLength, title, content, uploadingImages])
    ...
    try {
      ...
      const imageKeys =
        files.length === 0
          ? []
          : await (async () => {
              const presignedItems = await presignCommunityPostImageUploads(files)
    ...
    <section className="mx-auto max-w-5xl rounded-2xl border border-border bg-card p-6 shadow-sm">
      ...
      <div className="mt-6 grid gap-5 rounded-2xl border border-border bg-background p-5">
        ...
        <div className="grid gap-2">
          ...
          <div className="flex flex-wrap gap-2">
            <button
              ...
              disabled={creating || uploadingImages || images.length >= MAX_IMAGES}
        ...
        <div className="flex flex-wrap items-center justify-end gap-2">
          ...
          >
            {uploadingImages ? '이미지 업로드 중…' : creating ? '등록 중…' : '등록'}
          </button>
        </div>
      </div>
    </section>
